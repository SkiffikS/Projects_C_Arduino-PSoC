...
{
    CyRamVectors[CYINT_IRQ_BASE + IRS__INTC_NUMBER] = address;
}
...
{
    return CyRamVectors[CYINT_IRQ_BASE + IRS__INTC_NUMBER];
}
...
void IRS_SetPriority(uint8 priority)
{
    ...
    uint32 priorityOffset = ((IRS__INTC_NUMBER % 4u) * 8u) + 6u;
    
	interruptState = CyEnterCriticalSection();
    *IRS_INTC_PRIOR = (*IRS_INTC_PRIOR & (uint32)(~(uint32)IRS__INTC_PRIOR_MASK)) |
...
uint8 IRS_GetPriority(void)
{
    ...
    priority = (*IRS_INTC_PRIOR & IRS__INTC_PRIOR_MASK) >> priorityOffset;
