...
#define UART_BUFFER_PLUS_F_IDX (2) // PLUS - Symbol "+"
...
{
        ChannelController(FIRST_CHANNEL); // BLUE
        ChannelController(SECOND_CHANNEL); // GREEN
        ChannelController(THIRD_CHANNEL); // RED
}
// ==========================================================================================
...
void UartBufferParser(void)
{
 ...
 if ('+' != UartBuffer[UART_BUFFER_PLUS_F_IDX])
    {
        Error_Return();
        return;
    }
 ...
 if ('+' != UartBuffer[UART_BUFFER_PLUS_S_IDX])
    {
        Error_Return();
        return;
    }
    
 if ('+' != UartBuffer[UART_BUFFER_PLUS_T_IDX])
    {
        Error_Return();
        return;
    }
